...
use Frosh\WebP\Services\Generator\WebPGeneratorInterface;
use League\Flysystem\FileExistsException;
use Shopware\Core\Content\Media\Thumbnail\ThumbnailService as ThumbnailThumbnailService;
...
class ThumbnailService extends ThumbnailThumbnailService
{
    /**
    ...
    public function __construct(
        ...
        WebPGeneratorInterface $webPGenerator
    ) {
    ...
    public function updateThumbnails(MediaEntity $media, Context $context): int
    {
        ...
        */
        ...
        $this->generateMainImageWebP($media, $config);
    ...
    private function generateMainImageWebP(MediaEntity $media, MediaFolderConfigurationEntity $config)
    {
        ...
        $webP = $this->webPGenerator->generate($image, $config->getThumbnailQuality());
        imagedestroy($image);

        $path = pathinfo($path);
        $path = $path['dirname'] . '/' . $path['filename'] . '.webp';

        try {
            $fileSystem->write($path, $webP);
    ...
    private function writeThumbnail($thumbnail, MediaEntity $media, string $url, int $quality): void
    {
        ...
        $this->writeWebPImage($thumbnail, $media, $url, $quality);
    }

    private function writeWebPImage($thumbnail, MediaEntity $media, string $url, int $quality): void
    {
        $path = pathinfo($url);
        $newUrl = $path['dirname'] . '/' . $path['filename'] . '.webp';

        $this->getFileSystem($media)->put($newUrl, $this->webPGenerator->generate($thumbnail, $quality));
    }
